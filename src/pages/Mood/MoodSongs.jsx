  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4 bg-gray-900/50">
        <i class="w-8 h-8 text-green-500 animate-spin"></i>
        <div className="text-white text-lg font-medium">Loading your vibe...</div>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-gray-900 to-black  pt-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 space-y-4 sm:space-y-0">
          ...
          >
            <i class="fa-solid fa-play w-4 h-4 mr-2 transition-transform duration-200 group-hover:scale-110"></i>
            <span>Play</span>
          </button>
        </div>
        ...
        <div className="relative">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-4 md:gap-20">
            {songs.map((song) => (
              <div key={song.id} className="transform transition-transform duration-200 hover:-translate-y-1">
                <SongCard song={song} className="h-full rounded-xl shadow-lg shadow-black/20 hover:shadow-xl hover:shadow-black/30" />
              </div>
            ))}
          </div>
        </div>
